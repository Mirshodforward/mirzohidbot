...
from app.bot.states import EditStoreStates
...
from app.domain.store_flow import TZ_TASHKENT, fmt_datetime, fmt_money, parse_amount, parse_kw
from app.services.excel_stores import (
    single_store_debt_payments_excel_bytes,
    single_store_electricity_excel_bytes,
    stores_to_xlsx_bytes,
)
from app.services.stores import StoreError, apply_electricity_reading, apply_payment
...
async def _get_store_refreshed(sid: int) -> Store | None:
    ...
    async with async_session_maker() as session:
        s = await session.get(Store, sid, with_for_update=True)
        if not s:
            return None
...
async def _commit_kw_update(sid: int, new_k: int) -> tuple[str | None, dict | None]:
    """Hisoblagichni yozadi. Xato bo'lsa (matn, None), muvaffaqiyatda (None, ma'lumot).

    Haqiqiy mantiq `app.services.stores` da — API ham aynan shu funksiyani
    chaqiradi, shuning uchun bot va sayt orqali kiritilgan o'qim bir xil
    qoidalar bilan ishlanadi.
    """
    async with async_session_maker() as session:
        try:
            res = await apply_electricity_reading(session, sid, new_k)
    ...
    return (
        None,
        {
            "old_k": res.reading_before,
            "period_from_out": res.period_from,
            "now": res.period_to,
            "new_k": res.reading_after,
        },
    )
    ...
    async with async_session_maker() as session:
        try:
            res = await apply_payment(
                session, sid, raw_amt, created_by_telegram_id=admin_telegram_id
            )
        except StoreError as exc:
            await session.rollback()
            return (str(exc), None)
        await session.commit()
    return (None, (res.paid, res.debt_after))


async def _load_stores_ordered() -> list[Store]:
    now = datetime.now(TZ_TASHKENT)
    async with async_session_maker() as session:
        r = await session.execute(select(Store).order_by(Store.id.desc()).with_for_update())
...
async def edit_store_name_commit(message: Message, state: FSMContext) -> None:
    ...
    if not name:
        ...
        return
    if name in ALL_MENU_TEXTS:
        await message.answer(
            "Bu menyu tugmasining matni — magazin nomi sifatida saqlanmaydi.\n"
            "Yangi nomni yozing yoki <b>❌ Bekor qilish</b> tugmasini bosing.",
            parse_mode=ParseMode.HTML,
            reply_markup=cancel_keyboard(),
        )
        return
