...
from app.bot.states import AdminToStoreStates, BroadcastStates
from app.db.models import Store, StoreChatMessage, User
from app.db.session import async_session_maker
from app.domain.store_flow import normalize_phone
from app.services.tg_send import SendResult, broadcast, send_message_safe
...
async def _load_stores() -> list[Store]:
    from app.bot.handlers.admin_stores import _load_stores_ordered
...
async def broadcast_send(message: Message, state: FSMContext) -> None:
    ...
    if text in _MENU_TEXTS:
        ...
        if text == ADMIN_BTN_LIST:
            from app.bot.handlers.admin_stores import admin_list_stores

            await admin_list_stores(message)
            return
        if text == ADMIN_BTN_NEW:
            from app.bot.handlers.admin import add_store_begin

            await add_store_begin(message, state)
            return
        if text == ADMIN_BTN_REPORT:
            from app.bot.handlers.admin import report
    ...
    # Flood-control (429) hisobga olinadi: aks holda ~30 xabardan keyin
    # Telegram rad etadi va qolganlari jimgina yo'qoladi.
    ok, blocked, fail = await broadcast(message.bot, ids, text)

    lines = [f"Tugadi.\nMuvaffaqiyatli: {ok}"]
    if blocked:
        lines.append(f"Botni bloklaganlar: {blocked}")
    if fail:
        lines.append(f"Yuborilmadi (xato): {fail}")
    await message.answer("\n".join(lines), reply_markup=msg_mode_keyboard())
