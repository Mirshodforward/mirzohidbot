...
async def user_to_admin_send(message: Message, state: FSMContext) -> None:
    ...
    ok, fail = 0, 0
    for aid in admins:
        res = await send_message_safe(message.bot, aid, header, parse_mode=ParseMode.HTML)
...
async def owner_store_reply_commit(message: Message, state: FSMContext) -> None:
    ...
    ok = 0
    for aid in admins:
        if await send_message_safe(message.bot, aid, body, parse_mode=ParseMode.HTML) is SendResult.OK:
            ok += 1
