...
from app.db.global_tok_price import get_electricity_price_per_kw, set_electricity_price_per_kw
...
async def add_store_name(message: Message, state: FSMContext) -> None:
    ...
    if name in ALL_MENU_TEXTS:
        await message.answer(
            ...
            "Nomni yozing yoki <b>❌ Bekor qilish</b> tugmasini bosing.",
            parse_mode=ParseMode.HTML,
            reply_markup=cancel_keyboard(),
        )
        return
...
async def add_store_address(message: Message, state: FSMContext) -> None:
    ...
    if not addr:
        ...
        return
    if addr in ALL_MENU_TEXTS:
        await message.answer(
            "Bu menyu tugmasining matni — manzil sifatida saqlanmaydi.\n"
            "Manzilni yozing yoki <b>❌ Bekor qilish</b> tugmasini bosing.",
            parse_mode=ParseMode.HTML,
            reply_markup=cancel_keyboard(),
        )
        return
