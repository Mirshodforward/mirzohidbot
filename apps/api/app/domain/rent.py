...
def apply_rent_accrual_to_store(store: Store, now: datetime) -> bool:
    ...
    if cycles == 0:
        # Eski qatorlar (migratsiyadan oldin yaratilgan) uchun sikl 0 bo'lib qoladi.
        # Ilgari bu yerda `debt = m` turgan edi — mavjud qarzni butunlay o'chirib
        # yuborardi. Endi qarz bo'lsa saqlanadi, faqat bo'sh bo'lsa birinchi oy yoziladi.
        if debt <= 0:
            debt = m
        cycles = 1
        changed = True
...
async def refresh_all_store_rent_state() -> None:
    ...
    `with_for_update()` — qarz "o'qi → o'zgartir → yoz" ketma-ketligida. Fon
    vazifasi va foydalanuvchi so'rovi bir vaqtda kelsa, qulfsiz holda bir oylik
    summa ikki marta qo'shilib ketishi mumkin edi.
    """
