"""
Oxirgi DB zaxirasini (.sql) Telegram chatga yuborish.

`.env` ga qo'shing:
  BACKUP_BOT_TOKEN=<backup uchun bot token>
  BACKUP_SEND_CHAT_ID=5081646633

Ishlatish:
  python send.py
  python send.py --file backups/mirzohid_db_20260101_120000.sql
...
_root = str(Path(__file__).resolve().parents[1] / 'apps' / 'api')
...
def main() -> None:
    ...
    chat_id = _resolve_chat_id(get_settings().backup_send_chat_id)
